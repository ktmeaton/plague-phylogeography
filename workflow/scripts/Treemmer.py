...
def find_N(t, leaf):
    ...
    sister_flag = 0
    for n in range(
        0, len(parent.get_children())
    ):  ##this for loop start from parent and climb up max two nodes, if it finds leaves calculate the distances,
        if parent.is_root():
            flag = 1
            break
        if arguments.verbose == 3:
            ...
            if parent.children[n] != leaf:
                ...
                if arguments.verbose == 3:
                    print(
                        leaf.name
                        + ","
                        + parent.children[n].name
                        + str(DIS)
                        + "have one node of distance"
                    )
        else:
            if flag == 0:
    ...
    if flag == 0:  #### this means that the leaf has no neighbors at one node of dist
        parent = (
            parent.up
        )  #### therefore I climb the tree down towards the root of one more step and look for leaves
        multi_flag = 0
        if arguments.verbose == 3:
            print("going down")
            print("gran parent")
        ...
        for n in range(
            0, len(parent.get_children())
        ):  # this for loop start from gran parent and climb up max one nodes, if it finds leaves calculate the distances,
            if parent.is_root():
                break
        ...
        if multi_flag == 1:  # this is to deal with polytomies
            ...
            if arguments.verbose == 3:
                print(
                    ...
                    + parent.children[n].name
                    + str(DIS)
                    + "	are not sister taxa, but neighbours first is leaf, second is neighbor of downstair (towards root)"
                )
...
def check_protected(leaf_to_prune):
    ...
    for tag in dict_meta[leaf_to_prune]:  # loop thru all the tags of the selected leaf
        ...
        for k, v in dict_meta.items():  # loop thru all the leaves,tag
            ...
            if (
                flag == 1
            ):  # if the leaf has several time the same tag (mistake in input) it counts only one
        ...
        if arguments.list_meta_count:  # if -lmc option
            if int(
                dict_meta_count[tag] == []
            ):  # if the tag is not present in list_meta_count
...
def find_leaf_to_prune(
    dlist,
):  # parse the list with all neighbor pairs and distances, find the closest pair and select the leaf
    warning = 1
    while warning != 0:
        if len(dlist) == 0:
            leaf_to_prune = "stop,"
        ...
        if (
            arguments.list_meta and dict_meta[leaf_to_prune]
        ):  # check if leaf is protected
        ...
        if warning == 1:
            del dlist[
                pair_unsplit
            ]  # if both leaves of the pair are protected => delete the pair from dlist and make another cycle
...
def make_plot():
    ...
    plt.savefig(
        os.path.splitext(arguments.INFILE)[0]
        + "_res_"
...
parser.add_argument(
    "-pr",
    "--prune_random",
    help="prune random leaves (default: FALSE)",
    action="store_true",
    default=False,
)
parser.add_argument(
...
parser.add_argument(
    "-np",
    "--no_plot",
    help="do not load matplotlib and plot (default: FALSE)",
    action="store_true",
    default=False,
)
parser.add_argument(
...
parser.add_argument(
    ...
    "--select_all",
    default=False,
    help="output the list of leaf names in the input tree and exit",
    action="store_true",
)
parser.add_argument(
    "-pa",
...
if arguments.verbose > 0:
    ...
    if arguments.switch_at_X != 1:
        print(
            "\nWhen the tree is reduced to "
            + str(arguments.switch_at_X)
            + " leaves, Treemmer will switch behaviour and start pruning random leaves"
        )

    if arguments.switch_at_RTL:
        print(
            "\nWhen the tree is reduced to "
            + str(arguments.switch_at_RTL)
            + " of the original tree length, Treemmer will switch behaviour and start pruning random leaves"
        )
...
if (
    arguments.list_meta
):  ############# update the dictionary of taxa_names with tags, only taxa in the tree stays are kept in the dict
    for leaf in leaves:
...
while len(t) > 3:  #################### Main loop ################################
    ...
    DLIST = Parallel(n_jobs=arguments.cpu)(
        delayed(parallel_loop)(t, leaves, i) for i in range(0, arguments.cpu)
    ...
    for (
        ...
        DLIST
    ):  # when running in parallel DLIST is a dict of dicts, this for loop merge them all in one
        result.update(d)
    ...
    for r in range(
        ...
        if arguments.prune_random:
            ...
            (leaf_to_p) = find_leaf_to_prune_random(
                leaves
            )  # find leaf to prune,  protections (from -lm option) are embedded in the function
        else:
        ...
        if leaf_to_p == "stop,":
            if (
                r == 1
            ):  # if r > 1 some taxa might have been no considered because surrounded by already pruned leaves
                print(
        ...
        if leaf_to_p != "stop,":
            ...
            if (
                arguments.list_meta
            ):  ####### update the dictionary of taxa_names with tags, only taxa in the tree are kept in the dict
        ...
        if arguments.stop_at_X_leaves:  # if stop criterium is met (X) ==> output
            if (max(arguments.stop_at_X_leaves) >= len(t)) or (leaf_to_p == "stop,"):
                output1 = (
                    os.path.splitext(arguments.INFILE)[0]
                    + "_trimmed_tree_X_"
                    + str(max(arguments.stop_at_X_leaves))
                    + ".nwk"
                )
                output2 = (
                    os.path.splitext(arguments.INFILE)[0]
